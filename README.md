...
현재 플레이 루프는 `캠프 → 전쟁 계약 → 출전/지원/파견 편성 → Flame 전투 → 레벨업 → 전장 사건 선택 → 승리/후퇴/패배 → 보상 → F–S 승급/별 한계돌파/무기 강화 → 모집/상점 → 다음 출전`입니다. 신규 계정은 루나 한 명으로 시작하며 첫 계약에서 지원 용병 미라를 확정 영입합니다. 총 16명의 용병은 전투 8명·지원 4명·파견 4명으로 역할이 나뉩니다.
